...
async def execute_research_workflow(
    ...
    # Build result dictionary for evaluation
    result = {
        "quality_score": research.get('final_answer', {}).get('confidence', 'unknown'),
    ...
    # Convert quality_score from string to float if needed
    if isinstance(result["quality_score"], str):
        # Map confidence strings to numeric scores
        confidence_map = {"high": 0.9, "medium": 0.7, "low": 0.5, "unknown": 0.5}
        result["quality_score"] = confidence_map.get(result["quality_score"], 0.5)

    # Record metrics for this query
    evaluator.evaluate_query_result(result, execution_time)
